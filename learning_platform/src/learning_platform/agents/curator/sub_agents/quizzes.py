"""QuizzesAgent — generates MCQ quiz questions per lesson.
...
from learning_platform.models.agent_pipeline import QuizQuestion
...
AGENT_TYPE: str = "quizzes"

SYSTEM_PROMPT = """You are an educational quiz generation assistant.

Generate multiple-choice quiz questions from the lesson content provided.

Rules:
- Generate between 3 and 5 questions.
- Each question tests recall or conceptual understanding of the lesson content.
...
USER_PROMPT_TEMPLATE = """Generate quiz questions from the following lesson.
...
def _validate_question(entry: dict[str, Any], lesson_id: UUID) -> QuizQuestion | None:
...
class QuizzesAgent:
    """Generates 3–5 MCQ quiz questions per lesson using an LLM."""
    ...
    async def run(
        ...
        """Generate MCQ questions for a lesson. Raises on LLM failure."""
        ...
        if await completion_repo.has_completed(agent_process_id, lesson_id_str, AGENT_TYPE):
            _LOG.info("QuizzesAgent: already completed lesson %s, skipping", lesson_id)
            return []

        await output_repo.delete_quiz_questions_for_lesson(document_id_str, lesson_id_str)
        ...
        _LOG.info("QuizzesAgent: generated %d question(s) for lesson %s", len(items), lesson_id)
        return items
