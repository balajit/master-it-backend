"""FlashcardsAgent — generates AI flashcard pairs per lesson.
...
from learning_platform.models.agent_pipeline import AgentFlashcard
...
AGENT_TYPE: str = "flashcards"

SYSTEM_PROMPT = """You are an educational flashcard generation assistant.

Generate flashcard pairs from the lesson content provided.

Rules:
- Generate between 5 and 10 flashcard pairs.
- Each front should be a concise question or key term (≤15 words).
- Each back should be a clear, accurate answer or definition (≤50 words).
- Do NOT invent facts not supported by the lesson.
- Prefer concepts, definitions, and important facts.
- Return ONLY valid JSON matching this schema:
  {"flashcards": [{"front": "string", "back": "string"}]}
"""

USER_PROMPT_TEMPLATE = """Generate flashcards from the following lesson.
...
class FlashcardsAgent:
    """Generates 5–10 front/back flashcard pairs per lesson using an LLM."""
    ...
    async def run(
        ...
        """Generate flashcards for a lesson. Raises on LLM failure."""
        ...
        if await completion_repo.has_completed(agent_process_id, lesson_id_str, AGENT_TYPE):
            _LOG.info("FlashcardsAgent: already completed lesson %s, skipping", lesson_id)
            return []

        await output_repo.delete_flashcards_for_lesson(document_id_str, lesson_id_str)
        ...
        rows: list[AgentFlashcardRow] = []
        items: list[AgentFlashcard] = []
        for entry in raw_cards:
            ...
            if front and back:
                ...
                items.append(AgentFlashcard(lesson_id=lesson_id, front=front, back=back))
        ...
        _LOG.info("FlashcardsAgent: generated %d card(s) for lesson %s", len(items), lesson_id)
        return items
