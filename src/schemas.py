...
class URLRequest(BaseModel):
    """Request body for POST /api/courses/{course_id}/documents/convert-url.

    Accepts any http or https URL. Scheme validation is also enforced in the
    endpoint handler to give a clear error message for unsupported schemes.
    """

    url: str  # validated as AnyHttpUrl in the endpoint


class URLPDFConfirmRequest(BaseModel):
    """Request body for POST /api/courses/{course_id}/documents/confirm-url-pdf.

    References the pending PDF generated by convert_url_to_pdf via temp_id.
    An optional filename override replaces the URL-derived slug.
    """

    temp_id: str
    filename: Optional[str] = None  # if None, the derived slug is used
